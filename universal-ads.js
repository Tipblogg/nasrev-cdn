(function () {
    ...
    function getOrGeneratePPID() {
        ...
        try {
            ...
            if (!ppid) {
                ...
                const random = Math.random().toString(36).substr(2, 16); // Longer random string
                ppid = domain + '_' + random; // ✅ REMOVED TIMESTAMP
            ...
            // ✅ Try to persist in sessionStorage
            try {
    ...
    function checkValidationCache() {
        try {
            ...
            if (cached) {
                ...
                // ✅ FIXED: 5 minute cache (was 1 hour)
    ...
    function saveValidationCache(isValid) {
        try {
            ...
            log('Validation cached for 5 minutes', isValid);
        } catch (e) {
    ...
    function applyPublisherContext() {
        ...
        // ✅ FIXED: Category detection with generic keyword filter
        ...
        if (pubContext.category && typeof pubContext.category === 'string') {
            ...
            if (!category) {
                ...
                const categoryMatch = urlPath.match(/\/category\/([^\/]+)/) ||
                    ...
                    urlPath.match(/\/([^\/]+)\/\d{4}\//) || // /category/2025/post-name
                    urlPath.match(/\/blog\/([^\/]+)\//);
            ...
            if (category) {
                ...
                // ✅ FIXED: Filter out generic categories
    ...
    function configurePublisherAds() {
        ...
        // ✅ FIXED: More aggressive lazy loading on mobile
        ...
        // ✅ FIXED: Render event listener with proper refresh initiation
        pubads.addEventListener('slotRenderEnded', function (event) {
            ...
            // ✅ FIXED: Start refresh cycle immediately after render
    ...
    // ✅ FIXED: Refresh with error handling and retry logic
    function refreshSlot(slot, retryCount) {
        ...
        try {
            ...
            // ✅ CRITICAL: Update correlator for new ads
